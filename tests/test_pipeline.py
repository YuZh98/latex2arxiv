...
class TestApplyConfig:
    ...
    def test_replacements(self):
        result = apply_config(
            r"\added{new text}", {"replacements": [{"pattern": r"\\added\{([^}]*)\}", "replacement": r"\1"}]}
        )
        assert result == "new text"

    def test_replacements_bad_regex_does_not_crash(self, capsys):
        # Unbalanced bracket — should warn and skip, not raise.
        result = apply_config(r"\added{new text}", {"replacements": [{"pattern": r"[unclosed", "replacement": ""}]})
...
class TestPreflightChecks:
    ...
    def test_directory_warning_deduped(self):
        ...
        issues, _ = self._run(files)
        dir_space_warns = [w for w in issues.warnings if "directory" in w and "spaces" in w]
...
class TestFormatPdflatexErrors:
    ...
    def test_single_error_yields_exact_three_line_block(self):
        ...
        expected = "! Undefined control sequence.\nl.42 \\frobnicate\n                {arg}"
...
class TestInputResolution:
    ...
    def test_directory_input_full_pipeline(self, tmp_path):
        ...
        src.mkdir()
        (src / "main.tex").write_text(r"\documentclass{article}\begin{document}\includegraphics{fig}\end{document}")
...
class TestJsonOutput:
    ...
    def test_json_stdout_is_only_json(self, tmp_path):
        ...
        assert stripped.startswith("{") and stripped.endswith("}"), f"stdout is not pure JSON: {stripped[:200]!r}"
...
\end{document}""",
        ...
        assert content.count("MACROS") == 2, f"expected MACROS to appear twice; got {content.count('MACROS')} times"
...
class TestPreFlightChecks:
    def test_today_warns(self, tmp_path):
        ...
        assert any("today" in w.lower() for w in issues.warnings), f"Expected \\today warning, got: {issues.warnings}"
    ...
    def test_doublespacing_warns(self, tmp_path):
        ...
        issues = convert(Path(zp), Path(out), dry_run=True)
        Path(zp).unlink(missing_ok=True)
        assert any("double-spacing" in w.lower() or "doublespacing" in w.lower() for w in issues.warnings), (
...
class TestSubfileAndBibStyle:
    ...
    def test_subfile_with_bibliographystyle_warns(self, tmp_path):
        ...
        assert any("bibliographystyle" in w.lower() or "subfile" in w.lower() for w in issues.warnings), (
...
class TestSimpleYamlParser:
    ...
    def test_inline_mapping(self):
        result = self._parse("replacements:\n  - pattern: foo\n    replacement: bar\n")
