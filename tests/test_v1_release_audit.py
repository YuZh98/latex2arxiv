...
class TestJsonStringContracts:
    ...
    def test_issues_errors_have_no_bracket_prefix(self, tmp_path):
        ...
        zp = _zip(tmp_path, {"main.tex": r"\documentclass{article}\usepackage{minted}\begin{document}\end{document}"})
    ...
    def test_issues_warnings_have_no_bracket_prefix(self, tmp_path):
        ...
        zp = _zip(tmp_path, {"main.tex": r"\documentclass{article}\begin{document}\date{\today}\end{document}"})
        out = tmp_path / "out.zip"
        issues = convert(zp, out, dry_run=True)
...
class TestEpsPreFlight:
    ...
    def test_eps_file_warns(self, tmp_path):
        ...
        out = tmp_path / "out.zip"
        issues = convert(zp, out, dry_run=True)
        assert any("eps" in w.lower() for w in issues.warnings), f"Expected .eps warning, got: {issues.warnings}"
...
class TestSupportFilesPreserved:
    ...
    def test_custom_cls_preserved(self, tmp_path):
        ...
        with zipfile.ZipFile(out) as zf:
            assert "myjournal.cls" in zf.namelist(), "Custom .cls used by \\documentclass must be kept"
    ...
    def test_custom_sty_preserved(self, tmp_path):
        ...
        convert(zp, out)
        with zipfile.ZipFile(out) as zf:
            assert "mypackage.sty" in zf.namelist(), "Custom .sty used by \\usepackage must be kept"
    ...
    def test_unreferenced_sty_not_preserved(self, tmp_path):
        ...
        convert(zp, out)
        with zipfile.ZipFile(out) as zf:
            assert "unused.sty" not in zf.namelist(), "Unreferenced .sty must be pruned"
...
class TestConfigPathApplied:
    ...
    def test_config_warnings_appear_in_issues(self, tmp_path):
        ...
        assert any("unknown_key" in w for w in issues.warnings), f"Unknown config key must warn; got: {issues.warnings}"
...
class TestFlattenSubfileWrapper:
    ...
    def test_subfile_without_document_wrapper_warns(self, tmp_path):
        ...
        assert any(
            "missing" in w.lower() or "wrapper" in w.lower() or "begin{document}" in w for w in issues.warnings
...
class TestCommentStrippingVerbatim:
    ...
    def test_verb_url_not_stripped(self, tmp_path):
        ...
        with zipfile.ZipFile(out) as zf:
            src = zf.read("main.tex").decode()
        assert "% this is not a comment" in src, "Content inside \\verb| must not be stripped"

    def test_verbatim_environment_not_stripped(self, tmp_path):
        tex = (
            r"\documentclass{article}\usepackage{verbatim}"
            r"\begin{document}"
            r"\begin{verbatim}"
            "\n"
...
class TestUndefinedCitationDetection:
    ...
    def test_missing_citation_warns(self, tmp_path):
        ...
        assert any("nosuchkey" in w for w in issues.warnings), f"Undefined citation must warn; got: {issues.warnings}"
...
class TestJsonFieldCompleteness:
    ...
    def test_compile_field_is_always_null_in_v1(self, tmp_path):
        payload = self._json_payload(tmp_path)
        assert payload["compile"] is None, "compile field must be null in v1.0 (per json-schema.md contract)"
...
class TestBblPreservation:
    ...
    def test_bbl_with_different_stem_not_kept(self, tmp_path):
        ...
        convert(zp, out)
        with zipfile.ZipFile(out) as zf:
            assert "other.bbl" not in zf.namelist(), ".bbl with different stem must not be auto-preserved"
