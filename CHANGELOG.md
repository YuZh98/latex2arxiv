...
- Pre-flight: warn when the main `.tex` is shipped under a structural sub-directory (`src/`, `sources/`, `latex/`, `tex/`). Previously the auto-unwrap of a single top-level dir silently flattened such layouts and the "compiles from root" warn never fired (#174)
