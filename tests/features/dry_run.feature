Feature: Preview the run without producing output
  ...
  Scenario: --dry-run still reports the planned actions on stderr
    When I run `latex2arxiv paper.zip --dry-run`
    Then stderr summarises files that would be removed
    And stderr summarises files that would be kept
    And stderr contains a "[dry-run] No output written." style notice
  ...
  Scenario: --dry-run still runs all pre-flight checks
    ...
    When I run `latex2arxiv paper.zip --dry-run`
    Then both the warning and the error are emitted on stderr
